...
function setup(){
  ...
  text2.position(width +10, 310);
  text2.style("font-family", "sans-serif");
  text2 = createP("E + Click  : SET END   FOR SOLVER");
  text2.position(width +10, 330);
  text2.style("font-family", "sans-serif");  
  text2 = createP("Click  : MARK your path");
  text2.position(width +10, 250);
  text2.style("font-family", "sans-serif"); 
  text2 = createP("Space + Click  : UNMARK your path");
  text2.position(width +10, 270);
  text2.style("font-family", "sans-serif"); 
//not working
  // input = createInput();
...
function draw(){
  ...
  if(mouseIsPressed && !notSet){
    if(keyIsPressed && key =='s'){
      ...
      dest.end=true;}
      else if (keyIsPressed && key ==' ' && mouseX<width && mouseY>0 && mouseX>0 && mouseY<height)
      {grid[index(floor(mouseX/w),floor(mouseY/w))].marked=false;}
    else if(mouseX<width && mouseY>0 && mouseX>0 && mouseY<height)// just mark
      {grid[index(floor(mouseX/w),floor(mouseY/w))].marked=true;}
...
while(x<solveslider.value() && !notSet){
  if(dest){
    ...
    if(next && !found){
      if(next.i==dest.i && next.j==dest.j) {
        found = true;
        printResult();}
...
function printResult(){
  for(var i = 0; i< stack.length; i++)
    {var c=stack[i];
      if(!c.marked)
       createP("You din't mark cell : " + c.i + " , " +c.j)}
  for(var i = 0; i< grid.length; i++)
    {var c=grid[i];
      // if(c.marked && !contai(stack,c))
      //   createP(c.i + " "+ c.j + "extra")
    }
}

function contai(stack,c){
  for(var i = 0; i< stack.length; i++)
    {if (c===stack[i+1])
      return true;}
  ...
  this.show = function(){
    ...
    if(this.visited)
      {fill(0,250,250,200,200)
        if(this.marked) fill(0,250,50,160,200);
